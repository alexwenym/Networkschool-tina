...
c it uses the new multiplication factors for selected reaction types and rates from
...
c that will be changed into new_networksetup.txt file
      ...
      character*5 lab(nre),labb(nre), iso(nre), rtype(nre)
...
c read new_mult_factors.txt (fac(j) are the new multiplications factors for selected reaction rates)
   ...
   10 continue
      j = j+1
      read( 9,110, end=999) iso(j),rtype(j),fac(j)
  110 format(a5,1x,a5,1x,f8.4)
...
c write out new_networksetup.txt files
 ...
 21   CONTINUE
      ...
      do j=1,niso
         if(iso(j).eq.SPE1(L).and.rtype(j).eq.labb(L))
     &      rfactor=fac(j)
      end do
